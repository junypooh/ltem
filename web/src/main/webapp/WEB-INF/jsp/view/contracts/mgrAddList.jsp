    ...
    <div id="content">
        ...
        <div class="box_tbllist">
            <table summary="기업명,아이디,이름,휴대폰번호,권한,관리계약건,상태,변경일,가려진정보표시">
                ...
                <thead>
                <tr>
                    <th scope="col" class="per4"><input type="checkbox" name="allCheck" title="전체체크"></th>
                ...
                <tbody>
                    <c:forEach items="${managerList}" var="manager" varStatus="status">
                        <tr <c:if test="${status.last}">class="last"</c:if> id="${manager.mgrSeq}">
                            <td class="per4"><input type="checkbox" name="chkbox" value="${manager.mgrSeq}" title="체크"></td>
            ...
            <div class="btn_btmlist btn_lhid">
                <div class="btn_right">
                    <div class="btn_blk"><a href="javascript:void(0)" onclick="addManagers()">계정추가</a></div>
                </div>
            </div>
...
<div class="wrap_layerpop" id="failLayer" style="display: none;">
    <div class="layerpop pop2">
        ...
        <div class="box_contpop">
            <span class="txt_top mb10">추가할 계정을 선택해주세요.</span>
...
<div class="wrap_layerpop" id="succLayer" style="display: none;">
    <div class="layerpop pop2">
        ...
        <div class="box_contpop">
            <span class="txt_top mb10">계약 담당자가 추가 되었습니다.</span>
            <div class="btn_blk_full"><a href="${pageContext.request.contextPath}/contracts">확인</a></div>
        </div>
    </div>
</div>
<!-- // 하단 버튼한개 -->
...
<script type="text/html" id="accountListTemplate">
    {{each(i) managerList}}
        <tr {{if managerList.length == i+1}}class="last"{{/if}} id="\${mgrSeq}">
            <td class="per4"><input type="checkbox" name="chkbox" value="\${mgrSeq}" title="체크"></td>
...
<script type="text/javascript">
    ...
    function fn_search(currPage) {
        ...
        // 조회
        $.ajax({
            url: "${pageContext.request.contextPath}/contracts/managers/add/search",
            type: "GET",
            data : {serialNumber:'${serialNumber}', companyCd:'${companyCd}', status: $('#status').val(), column: $('#column').val(), searchWord: $('#searchWord').val(), page:currPage},
            async : false,
    ...
    function removeMask() {
        ...
        $.ajax({
            url: "${pageContext.request.contextPath}/contracts/inCharge/unmask",
            type: "POST",
    ...
    function addManagers() {
        ...
        $.ajax({
            url: "${pageContext.request.contextPath}/contracts/managers/add",
            type: "POST",
            data : {serialNumber:'${serialNumber}', mgrSeqs:mgrSeqs},
            async : false,
            success: function(result) {
                layerOpen('succLayer');
            },
